...
def fill_mesh(mesh2fill, file: str, num_aug, do_scale_verts, flip_edges_prct, slide_verts_prct):
    load_path = get_mesh_path(file, num_aug)
    if os.path.exists(load_path):
        mesh_data = np.load(load_path, encoding='latin1', allow_pickle=True)
    else:
        mesh_data = from_scratch(file, num_aug, do_scale_verts, flip_edges_prct, slide_verts_prct)
...
def from_scratch(file, num_aug, do_scale_verts, flip_edges_prct, slide_verts_prct):
    ...
    if num_aug > 1:
        faces = augmentation(mesh_data, do_scale_verts, flip_edges_prct, faces)
    build_gemm(mesh_data, faces, face_areas)
    if num_aug > 1:
        post_augmentation(mesh_data, slide_verts_prct)
...
def augmentation(mesh, do_scale_verts, flip_edges_prct, faces=None):
    if do_scale_verts:
        scale_verts(mesh)
    if flip_edges_prct:
        faces = flip_edges(mesh, flip_edges_prct, faces)
    return faces


def post_augmentation(mesh, slide_verts_prct):
    if slide_verts_prct:
        slide_verts(mesh, slide_verts_prct)
